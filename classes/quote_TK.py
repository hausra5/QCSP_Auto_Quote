...
class quote_app(tk.Tk):
    def __init__(self, count = 1, total = [], total_profit = [], df_name = None,
                ...
                customer_class = None, due_date = None, shirt_style = None, quantity_break = None,
                 s_q = None, m_q = None, l_q = None, xl_q = None,
                 xxl_q = None, xxxl_q = None, xxxxl_q = None, xxxxxl_q = None,
                ...
                quote_name = None, quote_dict = None):
        super().__init__()
    ...
    def create_quote_widgets(self):
        ...
        s_xl_q_label = tk.Label(self, text='S Quantity', font=('calibre', 12, 'bold'))
        s_xl_q_label.grid(row=4, column=0)
        s_xl_q_entry = tk.Entry(self, textvariable=self.s_q, font=('calibre', 12, 'normal'), width=27)
        s_xl_q_entry.grid(row=4, column=1)

        self.m_q = tk.StringVar()
        s_xl_q_label = tk.Label(self, text='M Quantity', font=('calibre', 12, 'bold'))
        s_xl_q_label.grid(row=5, column=0)
        s_xl_q_entry = tk.Entry(self, textvariable=self.m_q, font=('calibre', 12, 'normal'), width=27)
        s_xl_q_entry.grid(row=5, column=1)

        self.l_q = tk.StringVar()
        s_xl_q_label = tk.Label(self, text='L Quantity', font=('calibre', 12, 'bold'))
        s_xl_q_label.grid(row=6, column=0)
        s_xl_q_entry = tk.Entry(self, textvariable=self.l_q, font=('calibre', 12, 'normal'), width=27)
        s_xl_q_entry.grid(row=6, column=1)

        self.xl_q = tk.StringVar()
        s_xl_q_label = tk.Label(self, text='XL Quantity', font=('calibre', 12, 'bold'))
        s_xl_q_label.grid(row=7, column=0)
        s_xl_q_entry = tk.Entry(self, textvariable=self.xl_q, font=('calibre', 12, 'normal'), width=27)
        s_xl_q_entry.grid(row=7, column=1)

        self.xxl_q = tk.StringVar()
        xxl_q_label = tk.Label(self, text='2XL Quantity', font=('calibre', 12, 'bold'))
        xxl_q_label.grid(row=8, column=0)
        xxl_q_entry = tk.Entry(self, textvariable=self.xxl_q, font=('calibre', 12, 'normal'), width=27)
        xxl_q_entry.grid(row=8, column=1)

        self.xxxl_q = tk.StringVar()
        xxxl_q_label = tk.Label(self, text='3XL Quantity', font=('calibre', 12, 'bold'))
        xxxl_q_label.grid(row=9, column=0)
        xxxl_q_entry = tk.Entry(self, textvariable=self.xxxl_q, font=('calibre', 12, 'normal'), width=27)
        xxxl_q_entry.grid(row=9, column=1)

        self.xxxxl_q = tk.StringVar()
        xxxxl_q_label = tk.Label(self, text='4XL Quantity', font=('calibre', 12, 'bold'))
        xxxxl_q_label.grid(row=10, column=0)
        xxxxl_q_entry = tk.Entry(self, textvariable=self.xxxxl_q, font=('calibre', 12, 'normal'), width=27)
        xxxxl_q_entry.grid(row=10, column=1)

        self.xxxxxl_q = tk.StringVar()
        xxxxxl_q_label = tk.Label(self, text='5XL Quantity', font=('calibre', 12, 'bold'))
        xxxxxl_q_label.grid(row=11, column=0)
        xxxxxl_q_entry = tk.Entry(self, textvariable=self.xxxxxl_q, font=('calibre', 12, 'normal'), width=27)
        xxxxxl_q_entry.grid(row=11, column=1)

        self.location1 = tk.StringVar()
        location1_label = tk.Label(self, text='Location 1 Colors', font=('calibre', 12, 'bold'))
        location1_label.grid(row=12, column=0)
    ...
    def submit(self):
        ...
        self.quote_dict = {'S': int(self.s_q.get()),
                           'M': int(self.m_q.get()),
                           'L': int(self.l_q.get()),
                           'XL': int(self.xl_q.get()),
                           #'S-XL': int(self.s_xl),
                           '2XL': int(self.xxl_q.get()),
                           '3XL': int(self.xxxl_q.get()),
                           '4XL': int(self.xxxxl_q.get()), '5XL': int(self.xxxxxl_q.get())}
        ...
        quote_info = Quote_info(name=self.customer_class.name, email=self.customer_class.email,
                                ...
                                XXXXXL=float(self.xxxxxl_blank.get()), quantity_dict=self.quote_dict)
